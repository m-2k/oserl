...
accept(Sock) ->
    ...
    receive
      {_, CSocket, <<"\r\n">>} ->
          ...
          case parse_proxy_protocol_v2(<<"\r\n", ProxyHeader/binary>>) of
              {proxy, ipv4, _Protocol, Length} ->
                  ...
                  case ProxyAddr of
                      ...
                      _ ->
                          gen_tcp:close(Sock),
                          logger:error("Not proxy protocol"),
        ...
        {_, _CSocket, Data} ->
            logger:notice("data: ~p", [Data])
    after
        ?WAITING_TIMEOUT ->
            gen_tcp:close(Sock),
            logger:error("Proxy protocol header expected but not received"),
            {error, timeout}
    end.
...
pp2_client(<<0:5,             % UNASSIGNED
             _ClientCertSess:1,   % PP2_CLIENT_CERT_SESS
             _ClientCertConn:1,   % PP2_CLIENT_CERT_CONN
             _ClientSSL:1>>) ->
    client_ssl;
pp2_client(_) ->
    invalid_client.
